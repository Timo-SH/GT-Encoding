...
from data.ALCHEMY import Alchemy_loader
...
def run(config, create_dataset, create_model, train, test):
    ...
    for i in range(1, config.train_runs + 1):
        ...
        #Time logging and wandb initialization
...
if __name__ == "__main__":
    ...
    cfg.merge_from_file("models/config/alchemy.yaml")
    configuration = update_config(cfg)
    wandb.init(project="SAN_ZINC")
    loader_alchemy = Alchemy_loader(configuration)
    #wandb.config() #hier noch config übernehmen
    ...
    if configuration.model == "GT":
        ...
        run(config=configuration, create_dataset=loader_alchemy, create_model=create_SAN, train=train, test=test)