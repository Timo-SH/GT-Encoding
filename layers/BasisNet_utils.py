...
Describes the code of IGN networks adapted to pytorch from tensorflow
"""
import numpy as np
import torch
...
class layer_2_to_2(nn.Module):
    ...
    def __init__(self, input_depth, output_depth, normalization='inf', normalization_val=1.0, device='cpu'):
        ...
        self.coeffs = torch.nn.Parameter(
            torch.randn(self.input_depth, self.output_depth, self.basis_dimension) * np.sqrt(2.0) / (
                        self.input_depth + self.output_depth), requires_grad=True).to(device=self.device)
    ...
    def forward(self, inputs):
        ...
        mat_diag_bias = torch.eye(inputs.size(3)).unsqueeze(dim=0).unsqueeze(dim=0).to(
            device=self.device) * self.diag_bias
